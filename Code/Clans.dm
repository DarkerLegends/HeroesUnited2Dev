...
mob/proc/LeftClan(/**/)
	...
	if(!src.Clan)
		src.QuitClanWars("Left Clan")
		if(src.z==10)	src.JoinTerritoryWars()
...
mob/ClanManagement/verb
	...
	Set_Color()
		...
		if(ThisClan)
			...
			for(var/obj/TerritoryWars/Territory_Flag/T in world)	if(T.ClanOwner==ThisClan.name)	{T.ClanColor=ThisClan.Color;T.ColorClanFlag()}
