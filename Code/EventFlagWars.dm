...
mob/proc/DropFlag(var/Reason="Got Too Heavy?")
	...
	global.ClanWarsEntrants<<"[FlagWarsTag]<b>[src.GetClanTag()][src] has Dropped a <font color=[F.name]>[F.name] Flag! ([Reason])"
	F.ResetFlag()
...
obj/FlagWars
	Flags
		...
		verb/Capture()
			...
			global.ClanWarsEntrants<<"[FlagWarsTag] <b>[usr.GetClanTag()][usr] has Captured a <font color=[src.name]>[src.name] Flag!"
		proc/ResetFlag()
			if(src.CapturedBy)
			...
			src.CapturedBy=null
			spawn(-1)
				while(global.ClanWarsEntrants.len<(Flags2Spawn.Find(src.icon_state)-1)*6)	sleep(rand(10,20))
